...
if(!empty($_SESSION['LoggedIn']) && !empty($_SESSION['Login'])) {
	...
	if(isset($_POST["did"])){
		...
		$dsql_select = "SELECT j.regdate,ph.path,ph.mid FROM jurnal j JOIN ph_files ph on ph.mid=j.id WHERE ph.id = ".$did."";
		$dresult = mysql_query($dsql_select);
		...
		$s1 = "DELETE FROM ph_files WHERE id = '".$did."'";
