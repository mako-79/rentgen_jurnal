<?php include ("../base.php");
  ...
  if($level>3){
	...
	$query=mysql_query("SELECT u.id,u.Username,u.spec FROM users u JOIN groups g ON g.id=u.group_id WHERE LOWER(u.Username) LIKE LOWER('%".$s_fio."%') AND g.level=7");

	while($users=mysql_fetch_array($query) and $i<10){
		$matches[]=
		    array('value' => $users['id'],'label' => $users['Username'].", ".$users['spec']);
	}
